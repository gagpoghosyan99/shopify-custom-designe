...
if [[ -z "${SHOPIFY_CLI_THEME_TOKEN:-}" ]]; then
  ...
  echo "   Or run: shopify auth login"
  exit 1
fi
...
echo "→ NovaMeds theme dev (store: novameds.myshopify.com)"
exec shopify theme dev --store novameds.myshopify.com --password "$SHOPIFY_CLI_THEME_TOKEN" "$@"
